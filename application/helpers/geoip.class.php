...
define("GEODAT_FILE", __DIR__. DS .'geoIP.dat');
...
class GeoIP {
	public static function code($ip = ''){
		...
		//START GEOIP
			$returnArray = array();
			// Open the GeoIP data file
			
			$gi = geoip_open(GEODAT_FILE,GEOIP_STANDARD);
			// Get country code for ip
			$returnArray['IP'] =  $ip;
			$returnArray['COUNTRY_CODE'] =  geoip_country_code_by_addr($gi, $ip);
			$returnArray['COUNTRY_NAME'] = geoip_country_name_by_addr($gi, $ip);
			// Close GeoIP Handle;
			geoip_close($gi);
		//END GEOIP
		}
